...
namespace UdemyCourseProject.Services
{
    public class MessageSend : IEmailSend, ISmsSend
    {
        ...
        public Task SendEmailAsync(string email, string subject, string message)
        {
            ...
            myMessage.From = new MailAddress("jack@email.com", "Jack Johnson");
        ...
        {
            using (var client = new HttpClient { BaseAddress = new Uri("https://api.twillio.com") })
            {
                client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue(
                    "Basic", Convert.ToBase64String((Encoding.ASCII.GetBytes($"{Options.Sid}:{Options.AuthoToken}"))));
                ...
                var results = await
                    client.PostAsync($"/2010-04-01/Accounts/{Options.Sid}/Message.json",
                        contentSms).ConfigureAwait(false);
            }
        }
    }
}
